...
from total_ct.arteries import measure_aorta
...
class CTStats:
    ...
    def measure_vertebrae_stats(self):
        ...
        for ct in tqdm(self.ct_list):
            ct.vertebrae_stats = measure_vertebrae_body_and_diameter(ct, CTStats.total_map, self.device)
        print('Done!\n')
    ...
    def measure_tissue_stats(self):
        ...
        for ct in tqdm(self.ct_list):
            ct.tissue_stats = measure_ct_tissue_stats(ct, tissue_map, voi, self.device)
        print('Done!\n')
    
    def measure_aorta_stats(self):
        print('Measuring aortic diameter and calcification...')
        for ct in tqdm(self.ct_list):
            ct.aorta_stats = measure_aorta(ct, self.device)
    ...
    def _load_ct_variables(self):
        for ct in self.ct_list:
            ...
            ct.orig_img = torch.asarray(ct.orig_ct.get_fdata(), device=self.device, dtype=torch.int16)
            ...
            ct.total_seg = torch.asarray(nib.load(total_file).get_fdata(), device=self.device, dtype=torch.int8)
...
def measure_vertebrae_body_and_diameter(ct, class_map, device):
    cm_spacing = ct.pixel_measurements['pixel_spacing_cm']
    vert_bodies = torch.asarray(
        nib.load(f'{ct.seg_folder}/{ct.nii_file_name}_vertebrae_body.nii.gz').get_fdata(),
        device=device,
        dtype=torch.int8
    )
    verts = torch.where(vert_bodies == 1, ct.total_seg, 0)
    del vert_bodies
    ...
    for i in vert_range:
        ...
        arr = verts == i
        if arr.sum() != 0:
            z_indices = torch.where(arr)[-1]
            z_slices, frequencies = torch.unique(z_indices, return_counts=True)
            weighted_midline = int(torch.sum(z_slices * frequencies) / torch.sum(frequencies))
            major_axis, minor_axis = measure_binary_diameter(cp.array(arr[:, :, weighted_midline].cpu()), ct.pixel_measurements['y_x_spacing'])
            vert_vals[name] = {'midline': weighted_midline, 'major_axis_diameter_mm': major_axis, 'minor_axis_diameter_mm': minor_axis}
        else:
            vert_vals[name] = {'midline': None, 'major_axis_diameter_mm': None, 'minor_axis_diameter_mm': None}
    return vert_vals


def measure_ct_tissue_stats(ct, tissue_map, voi, device):
    ...
    for vert, midline in verts.items():
        vert_body = CTStats._largest_connected_component(body[:, :, midline].cpu())
...
def measure_binary_diameter(array, pixel_spacing):
    ...
    return smooth_diameters(major_dist, minor_dist)
...
def smooth_diameters(major_dist, minor_dist, threshold=15):
